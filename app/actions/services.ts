...
export async function updateService(serviceId: string, data: any) {
  ...
  // If base_price is being updated, also update price field to keep them in sync
...
export async function updateAllServicesPricing(percentage: number) {
  ...
  for (const service of services || []) {
    const currentPrice = service.price || service.base_price || service.provider_price * 3
