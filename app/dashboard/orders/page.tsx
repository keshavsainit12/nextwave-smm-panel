...
async function OrdersContent({ page = 1 }: { page: number }) {
  ...
  const { data: orders, error: ordersError } = await supabase
    .from("orders")
    .select("id, user_id, service_id, external_order_id, link, quantity, price, start_count, remains, status, can_refill, refill_count, created_at, services(id, name, icon, platform, has_refill, can_cancel, category_id)")
    .eq("user_id", user.id)
